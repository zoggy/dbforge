...
open Dbf_sql_driver
module Sqlite3Driver : SqlDriver =
struct
  type db     = Sqlite3.db
  type cursor = Mysql.stmt
  ...
  let escape_identifier = fun name ->
    if not (Str.string_match identifier_re name 0) then
      invalid_arg "Invalid Sqlite3 identifier";
  ...
  let check_for_exceptions = fun doit ->
    try  doit ()
    with
      Sqlite3.InternalError s ->
        raise (Sql_error (Printf.sprintf "Sqlite3 Internal error: %s" s)
    | Sqlite3.Error s ->
        raise (Sql_error (Printf.sprintf "Sqlite3 error: %s" s)
    | Sqlite3.Range_error (n,m) ->
        raise (Sql_error (Printf.sprintf "Sqlite3 range error: %d>%d" n m))

  let connect = fun ?host ?database ?port ?password ?user () ->
    let doit = fun () ->
      match database with
          None -> failwith "No database file given for sqlite3 to open"
        | Some file ->
            Sqlite3.db_open file
    in
    check_for_exceptions doit

  let disconnect = fun db ->
    check_for_exceptions (fun () -> Sqlite3.db_close db)
